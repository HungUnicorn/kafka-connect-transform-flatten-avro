...
/**
 * Flatten a nested data structure as the normal Flatten, and supports flatten nested optional null
 */
public abstract class FlattenAvro<R extends ConnectRecord<R>> implements Transformation<R> {

    private static final String DELIMITER_CONFIG = "delimiter";

    // "." is problematic as delimiter, change to "_" as default.
    ...
    private R applyWithSchema(R record) {
        // struct can be null, and will handle it later because we still need to build flattened avro schema.
        ...
        // flattened schema is build. return the value because it's null and no need to build it recursively
        if (value == null) {
        ...
        for (Field field : record.schema().fields()) {
            ...
            switch (field.schema().type()) {
                ...
                case BYTES:
                    ...
                    break;
                case STRUCT:
                    final Struct struct = record.getStruct(field.name());
                    if (struct != null) {
                        buildWithSchema(struct, fieldName, newRecord);
                    }
                    break;
                default:
                    throw new DataException("Flatten transformation does not support " + field.schema().type()
    ...
    public static class Key<R extends ConnectRecord<R>> extends FlattenAvro<R> {
        @Override
    ...
    public static class Value<R extends ConnectRecord<R>> extends FlattenAvro<R> {
        @Override
        protected Schema operatingSchema(R record) {
